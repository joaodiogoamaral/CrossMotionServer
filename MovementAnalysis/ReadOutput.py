...
def checkSquat(features,keys,plane):
	...
	if(plane=='left'):
		idx = keys.index('lHipKnee')
	elif(plane=='right'):
		idx = keys.index('rHipKnee')
	else:
		idx = keys.index('cHiprKnee')
		idx2 = keys.index('cHiplKnee')
	
	if(max(features[idx])>0):

		if(idx2==-1):
			return True
		elif(max(features[idx2])>0):
			return True
		else:
			return False
	else:
...
def getFeatures(keypointMatrix):
		...
		for key,i in features.items():
			...
			if(len(i)==0):
				...
				#print(normalizedFeatures[key])
				continue	

		
			
		#if(max(normalizedFeatures['rHipKnee'])): #or  max(normalizedFeatures['lHipKnee'])>0 ):
		#print(numpy.amax(normalizedFeatures['rHipKnee']))
		#storeModel.storeMovement(normalizedFeatures)

		#print(features.keys())
		extractedFeatures.append(features.keys())


		for i in features.keys():

			if(i == 'lShoulderHip' or i=='rShoulderHip'):

				print('FEATURE:'+i)
				print('MAX\n')
				print(max(processOutputs.normalize(features[i])))
				print('MIN\n')
				print(min(processOutputs.normalize(features[i])))

		

		
		#check if there is a squat
		#print(max(normalizedFeatures['']))
